...
# for non-target species in the gopher/black-and-yellow data set
...
# /Users/dianabaetscher/Desktop/NOAA_grad/git-repos/rockfish-species-id/nsf_data/rds_files
# dsb:rds_files dianabaetscher$ ls -l | awk 'BEGIN {print "gtseq_run", "file"} NR >1 && !/posinfo/ {num = $NF; gsub(/gtseq/, "", num); gsub(/[._].*$/, "", num);  print num, $NF}' > ../../nsf_data/rds-file-list.txt 

# get the names of the files
fdf <- read.table("nsf_data/rds-file-list.txt", stringsAsFactors = FALSE, header = TRUE) %>%
  tbl_df()
dir <- "nsf_data/rds_files"
...
# # save that.
# 
saveRDS(sample_sheets, "nsf_data/processed/sample-sheet-tibble.rds", compress = "xz")
# 
...
# and then save that
saveRDS(genos_long_explicit_NAs, file = "nsf_data/processed/called_genos_na_explicit.rds", compress = "xz")
