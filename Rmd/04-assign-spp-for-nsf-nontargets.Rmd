...
# look at the fish that were assigned with 95% confidence
nsf_assignments <- mix_assign$indiv_posteriors %>%
  filter(PofZ > 0.95) 

grp_tally <- nsf_assignments[,-10] %>% # drop the missing loci column 
  filter(z_score > -2 & z_score < 2) %>% # only within 2 standard deviations
  group_by(repunit) %>%
  tally()
  #write_csv("../nsf_data/spp_assignments_1-PofZ_zscore2.csv")

grp_tally2 <- grp_tally %>%
  arrange(desc(n)) %>%
  filter(n > 1)
...
```{r}
 mix_assign$indiv_posteriors %>%
  ...
  filter(collection %in% c("Scarnatus", "Schrysomelas", "Scaurinus")) %>%
  select(indiv) %>%
  rename(NMFS_DNA_ID = indiv) %>%
  write_csv("csv_outputs/nsf-GBYC-samples-ids.csv")
